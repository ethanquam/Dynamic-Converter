(function () {
  ...
  function setFromCubicMeters(cubicMeters) {
    if (!Number.isFinite(cubicMeters)) return;

    currentCubicMeters = cubicMeters;
    activeField = "cubicMeters";
    isUpdating = true;

    document.querySelectorAll("#volume-converter-grid .measure-row").forEach((el) => {
      el.classList.toggle("is-active", el.dataset.field === "cubicMeters");
      el.classList.remove("has-error");
    });

    Object.entries(FIELDS).forEach(([key, cfg]) => {
      document.getElementById(cfg.inputId).value = cfg.format(cubicMeters);
    });

    isUpdating = false;
    setStatus("Loaded from construction calculator.");

    const section = document.getElementById("volume-converter-heading");
    if (section) {
      section.scrollIntoView({ behavior: "smooth", block: "start" });
    }
  }

  window.VolumeConverter = { setFromCubicMeters };
})();
